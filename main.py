...
while(cap.isOpened()):
    ...
    emo = emo['emotions']
    #print(emo, end=' ')
    score = emo['happy']+emo['surprise']-emo['angry']-emo['disgust']-emo['fear']-emo['sad'] #расчёт, насколько понравилось пользователю видео
    #print(score)
    if score > 0.5 and liked == False: #проверка, понравилось ли пользователю видео
    ...
    if results.multi_hand_landmarks is not None:
        ...
        for i in range(0, len(positions)):
            if abs(positions[i][0][0] - positions[len(positions)-1][0][0]) < 100: #проверяем, что по оси x движение было коротким
                if positions[i][0][0] - positions[len(positions)-1][0][1] > 30 and positions[i][0][0] - positions[len(positions)-1][0][1] < 300 and "Right" in str(results.multi_handedness[0]): #проверяем движение вверх по оси y
                    keyboard.send("down") #идём к следующему видео
                    liked = False
                    break
                if positions[i][0][0] - positions[len(positions)-1][0][1] < -30 and positions[i][0][0] - positions[len(positions)-1][0][1] > -300 and "Left" in str(results.multi_handedness[0]): #проверяем движение вниз по оси y
